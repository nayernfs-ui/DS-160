...
async function runDocxSmokeTest() {
  ...
  try {
    ...
    // Generate without header (default behavior: no embedded header image)
    const { buffer, processedData } = await submit.generateDocument(sampleFormData, {
    ...
    if (!startsWithPK)
      throw new Error('Generated file does not seem to be a valid DOCX (missing PK signature)');
    ...
    const ReshaperExport =
      ReshaperModule.default || ReshaperModule.ArabicReshaper || ReshaperModule;
    let reshaper;
    try {
    ...
    const shaped =
      reshaper && typeof reshaper.reshape === 'function' ? reshaper.reshape(sampleVal) : sampleVal;
    ...
    if (!processedData || !processedData.FullName)
      throw new Error('ProcessedData was not returned by generateDocument');
    ...
    if (
      processedData.FullName !== expected &&
      processedData.FullName !== shaped &&
      processedData.FullName !== sampleVal
    ) {
    ...
    if (!foundReversed && !foundShaped && !foundRaw && !foundInOrder) {
      ...
      if (m) {
        ...
        console.error(
          'Found Arabic character at index',
          idx,
          'snippet',
      ...
      throw new Error(
        'Generated docx did not contain the Arabic text in any expected form (raw, shaped, reversed, or in-order)'
      );
    }
    ...
    // Verify header image embedding behavior explicitly: generate a DOCX WITH header image and ensure it's embedded
    ...
    if (origImageBuffer) {
      const bufferWithHeader = await submit.generateDocument(sampleFormData, {
        includeHeaderImage: true,
      });
      const zipWithHeader = await JSZip.loadAsync(bufferWithHeader);
      const mediaFiles = zipWithHeader.filter((relativePath) =>
        relativePath.startsWith('word/media/')
      );
      if (mediaFiles.length === 0) {
        throw new Error(
          'No images found under word/media in generated DOCX when includeHeaderImage=true; header not embedded'
        );
      }
      ...
      if (!foundMatch) {
        console.warn(
          '⚠️ Header image not exactly matched in DOCX media files — image might be embedded but modified or optimized. media files:',
          mediaFiles
        );
      }
    }
    ...
    const bufferNoHeader = await submit.generateDocument(sampleFormData, {
      includeHeaderImage: false,
    });
    const zipNoHeader = await JSZip.loadAsync(bufferNoHeader);
    const mediaFilesNoHeader = zipNoHeader.filter((relativePath) =>
      relativePath.startsWith('word/media/')
    );
    if (mediaFilesNoHeader.length !== 0) {
      console.warn(
        '⚠️ DOCX generated with includeHeaderImage=false still contains media files:',
        mediaFilesNoHeader.map((m) => m.name || m)
      );
