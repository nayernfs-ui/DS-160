...
(async () => {
  ...
  try {
    ...
    await browser.close();
    // exit with non-zero if widowed is visible when married should hide it
    const widowedVisible = info.widowedDisplay && info.widowedDisplay !== 'none';
    const marriedVisible = info.marriedDisplay && info.marriedDisplay !== 'none';
    if (marriedVisible && !widowedVisible) {
      console.log('PASS: Married visible and Widowed hidden');
      process.exit(0);
    } else {
      console.error('FAIL: Visibility mismatch', { widowedVisible, marriedVisible });
      process.exit(2);
    }
  } catch (err) {
