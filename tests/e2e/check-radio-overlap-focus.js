...
(async () => {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      const groups = Array.from(document.querySelectorAll('.question-group')).map((g, idx) => ({ el: g, idx }));
      const out = [];
      groups.forEach((gObj) => {
        const g = gObj.el;
        ...
        const focusable = g.querySelector("input:not([type='radio']):not([type='checkbox']), select, textarea, button");
        ...
        const checkOverlap = (contextNote) => {
          return radios.map((r) => {
            ...
            return { name: r.name, id: r.id || null, value: r.value, context: contextNote, overlapped, elementAtCenter: topEl && topEl.outerHTML ? topEl.outerHTML.slice(0,200) : null };
          });
        };
